...
def enforce_beat_synced_entrance_contract(
    ...
    is generated by H3 and stays continuous through the first four beats.
    """
    ...
    specs = [
        ...
        },
        {
            ...
            "environment_response": "Interior reflections fall away at the doorway as exterior campus air, daylight and open-space depth become clearly visible before the corner surface covers the image.",
            ...
            "additional_direction": corridor + " FACE-FIRST CAUSAL BRIDGE: the camera must see @P1's unobstructed frontal or three-quarter face and surprised expression before the full architectural wipe; no back-only exit, teleport, dissolve, identity replacement or renewed long entrance walk.",
        },
        {
            "preset": "P4 Slow-Motion Final Reveal", "framing": "Preserve @P4 subject count, identities, wardrobe and relative arrangement inside the established campus exterior",
            "camera_angle": "Eye level matching the established campus camera axis",
            "camera_movement": "Very slow horizontal slide through the established campus while keeping inherited camera height, horizon and viewing direction stable; then lock the Final Hold",
            ...
            "subject_action": "Reveal every @P4 subject already present inside the same campus exterior seen before the wipe while preserving exact @P4 faces, bodies, hair, wardrobe, accessories, subject count and arrangement, then use physical slow motion and settle before the Final Hold.",
            "environment_response": "Use the preceding campus as the sole environment. Relight and ground all @P4 subjects into its daylight, perspective and ground plane with shared contact shadows, colour temperature and atmospheric depth.",
            "continuity_state": "Inherit campus architecture, camera axis, daylight and ground plane from the incoming 24 frames. Preserve @P4 only as subject evidence. Hold the integrated campus composition during the final 0.5 second.",
            ...
            "incoming_environment_state": "CAMPUS CONTINUITY: the preceding 24 frames own the complete outdoor campus, camera axis, light and ground plane; @P4 supplies visible subjects only.",
            "outgoing_environment_state": "The integrated @P4 subjects, campus perspective, shared light, ground contact and shadows remain coherent through the locked final frame.",
            "location_transition": "ARCHITECTURAL WIPE TO CAMPUS COMPOSITE: clear the preceding wall wipe into the same campus and reveal the @P4 subjects already standing inside it.",
            "additional_direction": "P4 CAMPUS COMPOSITE. Treat @P4 only as the immutable authority for visible-subject identity, face, body, hair, wardrobe, accessories, subject count and relative arrangement. The incoming motion-reference frames are the sole authority for campus background, geometry, lighting, camera axis and ground plane. Integrate rather than replace either source.",
        },
    ]
    ...
    unrelated_uses = [
        ...
        and str(row.get("media_id", "")).strip().upper() not in {"P1", "P2", "P3", "P4", "A1"}
    ]
    ...
    plan["existing_media_uses"] = unrelated_uses + [
        ...
        image_use("beat_p1_campus_exit", "P1", cuts[3], cuts[4], "V1", "Return to exact P1 for the continuous interior-to-campus exit, corner eyeline and architectural wipe that causally introduces P4."),
        image_use("beat_p4_final", "P4", cuts[4], cuts[5], "V4", "P4 SUBJECT COMPOSITE: use exact P4 visible-subject identities, faces, bodies, hair, wardrobe, accessories, subject count and relative arrangement only. Integrate them into the campus environment inherited from the preceding 24 motion-reference frames; do not use P4 as a background plate."),
        {
    ...
    contract = (
        ...
        "reaction before a complete architectural wipe. Its final 24 frames alone own the ending's campus, "
        "while P4 supplies only the final subjects and their arrangement; discard the P4 source background. "
    ...
    notice = "Repaired Beat-Synced Entrance roles: corridor models are slow and face-readable; P3 is the third featured subject; the P1 bridge is face-first; P4 subjects are integrated into inherited campus continuity; A1 is source-windowed per Segment."
...
def normalize_design_plan(
    ...
    *,
    ...
    if str(special_skill_key or "").strip().casefold() == BEAT_SYNCED_ENTRANCE_SPECIAL_SKILL:
        # All Pictures are already supplied and H3 creates the corridor itself.
        # Never waste Z-Image calls on replacement subjects or a corridor plate.
        plan["media_requests"] = []
