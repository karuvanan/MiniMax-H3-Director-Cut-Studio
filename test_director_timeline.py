...
class DirectorTimelineDragTests(unittest.TestCase):
    ...
    def test_mtv_hidden_segments_never_exceed_seven_seconds(self):
        window = DirectorCutStudio()
        ...
        self.assertEqual(
            [(row.start_seconds, row.end_seconds) for row in segments],
            [(0.0, 6.0), (6.0, 12.0), (12.0, 18.0)],
        )
        self.assertTrue(all(row.duration_seconds <= 7.0 for row in segments))
        window.project_dirty = False
        window.close()
    ...
    def test_saved_mtv_prompt_removes_guessed_mouth_timing_without_lyrics(self):
        ...
        window.director_cues = [
            DirectorCue(
                "S1", "shot", 0.0, 6.0, "Performance",
                subject_action=(
                    "P1 sings the climactic phrase from A1, mouth open on the phrase, "
                    "mouth closed between phrases."
                ),
            )
        ]
        spec = window._prompt_spec_with_director_cues(
            window.prompt_panel.spec(),
            window_start=0.0,
            window_end=6.0,
            is_final_window=False,
        )
        rendered = " ".join(spec.shots).casefold()
        self.assertNotIn("climactic phrase", rendered)
        self.assertNotIn("mouth closed between phrases", rendered)
        window.project_dirty = False
        window.close()

    def test_mtv_final_reference_window_uses_tail_then_silence_padding(self):
        window = DirectorCutStudio()
        index = window.special_combo.findData("mtv-singing-h3")
        self.assertGreaterEqual(index, 0)
        window.special_combo.setCurrentIndex(index)
        root = PROJECT_ROOT / ".director_cache" / "test_mtv_final_audio_window"
        source = root / "master_a1.mp3"
        cache_root = root / "cache"
        root.mkdir(parents=True, exist_ok=True)
        source.write_bytes(b"source-audio")
        ...
        asset = MediaAsset(
            node_id="audio-loader", class_type="LoadAudio", media_type="audio",
            filename=source.name, binding="A1", local_path=str(source),
            start_seconds=0.0, end_seconds=99.402, timeline_placed=True,
            source_duration_seconds=99.402, source_in_seconds=0.0,
            source_out_seconds=99.402, reference_id="A1",
        )
        commands = []
        ...
        with patch("director_cut_studio.CACHE_ROOT", cache_root), patch(
            ...
            prepared = window._prepare_windowed_reference_audio(
                [asset], 98.0, 99.5
            )

        command = commands[0]
        self.assertEqual(command[command.index("-ss") + 1], "98.000000")
        self.assertIn(
            "atrim=duration=1.402000,apad=whole_dur=1.500000",
            command,
        )
        self.assertEqual(prepared[0].source_out_seconds, 1.5)
        self.assertNotEqual(prepared[0].local_path, str(source))
        window.project_dirty = False
        window.close()
