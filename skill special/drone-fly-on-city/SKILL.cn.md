...
仅在有助于连续性时，创建路线中点、环绕目标或结尾画面的参考图。先创建未命名的 Z-Image 媒体请求；结果实际加载后，才使用下一个可用的 `@P3` 至 `@P9` 标签。

每张延续图只能以 `@P1` 为唯一视觉母版，严格保持完全相同的地点、布局、地标、建筑、物件、天气、时间、光向、光强、色板、调色、曝光、对比度、氛围、镜头特性和特效；只允许改变符合航线的机位、取景或目标视角。`@P2` 永远不得作为视觉母版。最终 H3 参考集可含 `@P1` 与已加载的 `@P3`–`@P9`，对 `@P2` 必须为零视觉使用。
