...
def generate(job: dict) -> dict:
    ...
    for number, item in enumerate(requests, 1):
        ...
        emit({
            "progress": f"Generating reference image {number}/{len(requests)} · {destination.name}",
            "index": number - 1,
        })
        try:
            ...
            image_output = next(
                (output for output in outputs if str(output.get("filename", "")).lower().endswith((".png", ".jpg", ".jpeg", ".webp"))),
                None,
            )
            ...
            warning = f"{destination.name}: {exc}"
